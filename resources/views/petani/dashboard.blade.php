...
@section('content')
  <div class="container mx-auto min-h-screen flex flex-col items-center justify-start">
    <div class="w-3/4 px-6 mt-20 mb-10">
      ...
      <div class="card bg-base-200 mb-4">
        <div class="card-header woodImage p-5">
          <div class="flex items-center justify-between">
            <div>
              <h1 class="text-3xl font-bold text-slate-100 mb-1">🌾 Dashboard Usaha Tani</h1>
              <span class="text-sm text-slate-100">Selamat datang, {{ Auth::user()->name }} —
                {{ now()->isoFormat('dddd, D MMMM Y') }}</span>
            </div>
            <div class="flex gap-2">
              <a href="{{ route('petani.createProduksi') }}" class="btn btn-sm btn-warning">
                <i class="ti ti-tractor"></i>
                Input Produksi
              </a>
              <a href="{{ route('petani.catatKeuangan') }}" class="btn btn-sm greenImage">
          ...
          <div class="card-body p-0">
            <div class="flex items-center justify-between border-b border-base-content/10 px-4 py-3">
              <div class="flex items-center gap-2">
                ...
                <h3 class="font-semibold">Pendapatan vs Pengeluaran per Musim</h3>
              </div>
            ...
            <div class="p-4">
              <canvas id="grafikMusim" height="130"></canvas>
            </div>
          </div>
        </div>
            ...
            <div class="p-4 flex flex-col gap-3">
              <div class="rounded-lg bg-base-200/60 p-3 text-sm flex flex-col gap-2">
                ...
                <div class="flex justify-between font-bold text-base">
                  ...
                  <span class="{{ $saldoAkhir >= 0 ? 'text-info' : 'text-error' }}">
                    Rp {{ number_format($saldoAkhir, 0, ',', '.') }}
                  </span>
                </div>
              </div>
              <a href="{{ route('petani.laporanKeuangan') }}" class="btn btn-sm greenImage w-full">
                  ...
                  @forelse ($riwayatProduksi as $item)
                    <tr>
                      ...
                      <td class="text-end font-medium text-error">
                        Rp
                        {{ number_format($item->pendapatan?->labaRugi?->total_pengeluaran_produksi ?? 0, 0, ',', '.') }}
                      </td>
          ...
          <div class="card-body p-0">
            ...
            <div class="p-4 flex flex-col gap-2">
              @forelse ($transaksiTerbaru as $trx)
                <div class="flex items-center justify-between py-1.5 border-b border-base-content/5 last:border-0">
                  <div class="flex items-center gap-2">
                    <span
                      class="size-7 rounded-full flex items-center justify-center shrink-0
                      {{ $trx->jenis === 'pemasukan' ? 'bg-success/20' : 'bg-error/20' }}">
                      <span
                        class="size-3.5 {{ $trx->jenis === 'pemasukan' ? 'icon-[tabler--arrow-down] text-success' : 'icon-[tabler--arrow-up] text-error' }}"></span>
                    </span>
                    <div>
                      <p class="text-sm font-medium leading-tight">{{ $trx->kategori }}</p>
                      <p class="text-xs text-base-content/40">
                        {{ \Carbon\Carbon::parse($trx->tanggal)->format('d M Y') }}
                      </p>
                    </div>
                  </div>
                  <span class="font-semibold text-sm {{ $trx->jenis === 'pemasukan' ? 'text-success' : 'text-error' }}">
